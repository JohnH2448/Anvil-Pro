...
The issuer is decoupled from operand select by a very small issue queue. This queue is not intended to behave like a general dispatch buffer or replay window; it exists to absorb short operand-select holds without forcing the entire decode legality cone to sit on the same timing boundary as operand muxing and forwarding. Entries are still produced only after satisfying the normal issue contract, and the queue is flushed on redirect. Because stale source metadata may age while an entry waits, the queue repairs captured old-status state using completion, memory-ready, and retire events before operand select consumes it. This keeps the issue-governed model intact while making the timing boundary less brittle.
...
Blocking instructions follow a different path. Rather than completing at a limited latency inside the main execution fabric, they are handed off to a separate buffered execution structure. Once issued, they may remain in flight for an arbitrary amount of time while younger fixed-latency instructions continue to execute. When they eventually finish, they write their results back to the reorder buffer out of issue order. In Anvil-Pro, these are memory operations. It is worth noting that any long latency operation, say a multiply or divide, can also slot into this classification and reap benifits.

This distinction is where the reorder buffer becomes central to the backend organization. Although the machine remains architecturally in-order, the reorder buffer allows completion and retirement to be separated. Blocking and non-blocking operations may finish at different times, but the reorder buffer preserves correctness by holding results, providing forwarded data for completed but uncommitted instructions, and guaranteeing in-order commit. In this sense, the backend supports a constrained and purpose-built form of out-of-order completion without typically associated complexity.

Because of this structure, the issuer treats non-blocking and blocking instructions differently. Non-blocking instructions benefit from the backend’s limited-latency assumptions and can be issued permissively. Blocking instructions require a more conservative issue policy, since unresolved memory operations can create dependencies are more difficult to reapir. The design therefore attempts to hide the latency of blocking instructions through buffering, but when true dependencies arise, progress is frozen.

Additionally, if EX/EX bypass is enabled, data can be forwarded directly from the output of slot 0’s ALU to one or both input operands of slot 1’s ALU. This allows specific same-cycle inter-slot dependencies to be handled explicitly, which is a notable performance boost when hardware timing allows for it.
...
For FPGA timing, the restore path is allowed to cross an explicit registered boundary inside the ROB. Redirect detection still defines the architectural flush event, but the high-fanout restore request and restore buses are staged before driving the RST and CSR repair logic. This is a deliberate implementation tradeoff: the redirect mechanism remains derived from ROB metadata rather than checkpoints, while the physical path no longer requires a single large combinational cone to compute and distribute all restore state in one cycle.

Pipeline invalidation is simple. On redirect, all younger instructions in the pipeline are invalidated. This prevents any edge-case architectural state change and ensures that no speculative work can take effect.

### Forwarding Network
Anvil-Pro dedicates an entire pipeline stage to the multiplexing and selection of operands. The operand-select stage considers age-ordered set of candidate data sources. Each source operand can come from one of five places: the register file, the reorder buffer entry identified by the relevant age tag, load bypass, slot 1 execute, or slot 0 execute.

Selecting the correct producer in the presence of multiple in-flight candidates is the responsibility of the Register Status Table. The RST is deliberately engineered to reflect, from the perspective of operand-select pipeline time, where each architectural register should source from at that exact moment. It tracks the current age-wise owner of each register result, along with whether that result is still in flight, already produced, or fully committed. Operand select interprets this state to arrive at a final source decision. By construction, and under the guarantees established by the issuer contract, one of these candidate sources is always valid for non-blocking instructions. 
...
The execute bypass interface is kept explicit. Execute produces dedicated bypass data and age tags for the forwarding network rather than requiring operand select to infer them from broader result payloads. This does not change the forwarding contract, but it keeps the physical bypass path narrower and makes the producer identity presented to operand select unambiguous.

There is also an orange arrow from the memory queue to the upper execute slot. This corresponds to store-load forwarding, which is handled separately from ordinary register-result forwarding. Recent stores are held in a small store buffer, allowing younger dependent loads to source their data directly without waiting for the full external memory round trip. This substantially reduces effective load latency and prevents unnecessary pressure on the memory queue.
...
The old-status path is also aged while it is carried into operand select. A stale operand deliberately ignores the live RST entry, but the captured entry cannot be treated as frozen. The producer it names may still complete, return from memory, or retire while the consuming instruction is waiting to leave issue or is being held by an operand-select stall. For this reason, same-cycle ready, memory-ready, and retire events are compared against the captured age tag and folded into the old-status copy before it is consumed.
This allows the stale path to continue describing the same older producer while still tracking where that producer currently lives. If the producer completes but has not retired, the captured state becomes ready but uncommitted, causing operand select to use the ROB or load-bypass path. If the producer retires, the captured state becomes committed, causing operand select to use the register file. Without this repair, a stalled self-referential instruction could repeatedly make its forwarding decision from an out-of-date snapshot and either wait unnecessarily or select the wrong source.
...
### Memory
Memory operations are the only standard blocking operations in Anvil-Pro. The lower issue slot is forbidden from issuing memory work, so all loads and stores enter the memory subsystem through the upper lane. This restriction simplifies ordering, keeps the LSU single-ported from the pipeline's perspective, and avoids a second set of memory dependency checks in the lower lane.

After execute computes the effective address, memory operations are handed to the memory queue. The memory queue owns the long-latency interaction with the external data bus and returns completed loads to the ROB out of issue order. Stores are tracked through the store-buffer path so younger loads can receive forwarded data when the address and width allow it. This lets ordinary ALU work continue while memory is outstanding, while still preserving in-order architectural retirement through the ROB.

The memory queue also predecodes the target region for each request. Main data memory and CLINT/MMIO selection are captured with the queued memory payload, then consumed by the bus arbitrator. This avoids repeatedly placing the address-region decode on the late bus-return mux path and more closely matches the intended architectural split: the queue decides what kind of memory transaction it is holding, and the arbitrator only chooses the corresponding response source.

## Implimentation
The repository keeps the simulation/debug top and the FPGA synthesis top separate. The normal top-level path remains useful for Verilator and testbench visibility, while `SynthTop` is the Vivado-facing integration point. `SynthTop` exposes the data-memory Wishbone-style bus at the top level and uses a deliberately tiny BRAM-backed instruction memory. This prevents synthesis from optimizing the CPU away while keeping instruction memory from dominating resource and timing analysis.

The Artix-7 flow lives under `Synthesis/`. Reports are checked in under `Synthesis/Reports`, while checkpoints and logs are treated as local generated artifacts. The synthesis configuration is intentionally smaller than some simulation/performance exploration configurations, with a compact reorder buffer and tiny IMEM so implementation results describe the core timing problem rather than a large memory image.
