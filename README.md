...
### Status
The ALU instruction pipeline does strong even under directed edge case tests. Redirects are also now feeling much closer to a confident standard. Stress tests combining chained RAW, branch-after-branch, rst confusion, forwarding torture, store ordering, poison-path control flow, signed and unsigned memory accesses, and even awkward cases like loads into x0 all came back positive under Verilator. While not assuredly "100%", the foundation is strong enough to move towards other goals. The design is currently running at 1.58 IPC on standard (non-memory/system) assembly, and 1.98 IPC on non-dependent ALU assembly. The memory subsystem has improved meaningfully. Store retirement is no longer tied directly to waiting on external DMEM acknowledgement, and the current queue based approach appears functionally sound under difficult directed tests. A small classification mechanism was also added so writeback-to-x0 instructions with real architectural side effects, such as branches, jumps, loads, and stores, are not accidentally treated like harmless standard ops. That said, performance is still being held back by the lack of a true store buffer and load forwarding path. The current memory system now looks decent enough to stop being the obvious weak point, but broader verification and long-form program coverage are still needed. This is still the current undertaking, and shortly after that comes the store buffer.
